...
import {
  detectIngredientInventoryIntent,
  detectIngredientIntent,
} from "./ingredientIntent";
import { detectReportIntent } from "./reportIntent";
...
function sanitizeMessageForIntent(rawMessage = ""): string {
...
function detectNutritionTopic(text: string): string | null {
...
function detectNutritionIntent(
  text: string,
  normalized: string,
): Intent | null {
  ...
  if (!productRef) {
    productRef = text
      .replace(
        /\b(kandungan|gizi|nutrisi|kalori|energi|protein|karbohidrat|karbo|lemak|gula|serat|vitamin|mineral|untuk|pada|di|dari|berapa|apa|menu|makanan|minuman)\b/gi,
        " ",
      )
      .replace(/\s+/g, " ")
      .trim();
...
export function detectIntent(userMessage: string): Intent {
  ...
  const ingredientInventory = detectIngredientInventoryIntent(text, normalized);
  if (ingredientInventory) return ingredientInventory;
  ...
  const ingredient = detectIngredientIntent(text, normalized);
  ...
  const favRemove = normalized.match(
    /^(hapus|remove)\s+(?:menu\s+)?(?:favorit|favorite)\s+(.+)$/i,
  );
  if (favRemove)
    return { type: "favorite_remove", productRef: favRemove[2].trim() };
  const favAddNatural = text.match(
    /^tambah(?:kan)?\s+(.+?)\s+(?:sebagai\s+)?(?:menu\s+)?favorit(?:\s+saya)?$/i,
  );
  if (favAddNatural)
    return { type: "favorite_add", productRef: favAddNatural[1].trim() };
  const favAddShort = normalized.match(
    /^(favorit(?:kan)?|favorite(?:kan)?)\s+(.+)$/i,
  );
  if (favAddShort)
    return { type: "favorite_add", productRef: favAddShort[2].trim() };
  if (/favorit/i.test(text)) {
    const favByPossession = text.match(
      /^(?:halo\s+)?tambah(?:kan)?\s+(?:favorit\s+saya\s+)?(.+)$/i,
    );
    if (favByPossession) {
      return {
        type: "favorite_add",
  ...
  // Help
  if (
    ["halo", "hallo", "haloo", "hai", "hi", "p", "help", "bantuan"].includes(
      normalized,
    )
  )
  ...
  // Cart
  if (["keranjang", "cart"].includes(normalized)) return { type: "cart_view" };
  if (["checkout", "check out"].includes(normalized))
    return { type: "checkout", orderType: "pickup" };
  if (
    ["checkout delivery", "checkout antar", "checkout diantar"].includes(
      normalized,
    )
  )
