...
export type IngredientIntent =
  ...
  | { type: "ingredient_by_category"; category: string }
  | { type: "ingredient_single_stock"; ingredientRef: string }
  | { type: "ingredient_summary" }
  | { type: "ingredients_all"; category: string | null }
  | { type: "ingredients_single"; productRef: string };

export function capitalizeCategory(value = ""): string {
  const l = String(value ?? "").toLowerCase();
  if (l === "snack") return "Snack";
  if (l === "makanan") return "Makanan";
  if (l === "minuman") return "Minuman";
  return value;
}

export function normalizeIngredientReference(rawText = ""): string {
  return String(rawText ?? "")
    .toLowerCase()
    .replace(
      /\b(stok|bahan|cek|ada|berapa|jumlah|dong|ya|nih|please|tolong)\b/gi,
      " ",
    )
...
export function detectIngredientInventoryIntent(
  text: string,
  normalized: string,
): IngredientIntent | null {
  const hasBahan =
    /(bahan\s+pokok|stok\s+bahan|inventori\s+bahan|inventory\s+bahan|bahan\s+stok)/i.test(
      normalized,
    );
  const asksLow =
    /(bahan\s+menipis|stok\s+menipis|stok\s+habis|bahan\s+habis|stok\s+kritis)/i.test(
      normalized,
    );
  const asksPopIce =
    /(pop\s*ice).*(rasa|tersedia|stok)|rasa\s+pop\s*ice|pop\s*ice\s+tersedia/i.test(
      normalized,
    );
  const asksAll =
    /(apa\s+aja|apa\s+saja|semua|seluruh|list|daftar).*(bahan|stok)|bahan\s+pokok\s+yang\s+ada/i.test(
      normalized,
    );
  const asksOut =
    /(sudah\s+habis\s+berapa|berapa\s+yang\s+habis|jumlah\s+bahan\s+habis|habis\s+berapa)/i.test(
      normalized,
    );

  if (asksPopIce) return { type: "ingredient_pop_ice" };
  if (asksOut) return { type: "ingredient_out_summary" };
  if (asksAll) return { type: "ingredient_all_stock" };
  if (asksLow) return { type: "ingredient_low_stock" };

  const catMatch = normalized.match(/\b(makanan|minuman|snack|snacks)\b/i);
  const category = catMatch ? catMatch[1].toLowerCase() : "";
  if (hasBahan && category) return { type: "ingredient_by_category", category };
  ...
  if (hasBahan) return { type: "ingredient_summary" };
  return null;
}
