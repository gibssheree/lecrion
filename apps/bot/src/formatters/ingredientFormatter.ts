...
const INGREDIENT_RULES: Array<{ pattern: RegExp; ingredients: string[] }> = [
  {
...
export function inferIngredients(product: Product): string[] {
  const combined =
  ...
  if (!set.size) {
    if (product.category === "Minuman")
      ["air", "gula", "es batu (opsional)", "bahan utama minuman"].forEach(
        (i) => set.add(i),
      );
    else if (product.category === "Snack")
      ["bahan utama snack", "tepung", "gula/garam", "minyak goreng"].forEach(
        (i) => set.add(i),
      );
    else
      [
...
export function formatIngredientReply(product: Product | null): string {
  if (!product)
...
export function formatAllIngredientsReply(
  products: Product[],
  ...
  grouped.forEach(({ cat, items }) => {
    lines.push("", `${cat}:`);
    items.forEach((p, i) =>
      lines.push(`${i + 1}. ${p.name}: ${inferIngredients(p).join(", ")}`),
    );
  });
  lines.push(
    "",
    "Catatan: Komposisi bersifat estimasi dari data menu yang tersedia.",
  );
  return lines.join("\n");
}

export function formatIngredientInventorySummaryReply(summary: any[]): string {
  if (!summary.length) return "Data bahan pokok belum tersedia.";
  return [
    "📦 Ringkasan bahan pokok:",
    ...summary.map(
      (item) =>
        `- ${item.category}: ${item.totalProducts} produk | stok total ${item.totalStock} | menipis ${item.totalLowStock} | habis ${item.totalOutOfStock}`,
    ),
    "",
...
export function formatAllIngredientStocksReply(
  items: IngredientItem[],
  globalStats?: any,
): string {
  if (!items.length) return "Data stok semua bahan belum tersedia.";
  const lines = ["📚 Daftar semua stok bahan pokok:"];
  if (globalStats)
    lines.push(
      `Ringkasan: total produk ${globalStats.totalProducts} | total stok ${globalStats.totalStock}`,
    );
  items.forEach((item, i) =>
    lines.push(
      `${i + 1}. [${item.category ?? "-"}] ${item.name} - ${item.stock} ${item.unit ?? ""}`,
    ),
  );
  return lines.join("\n");
}

export function formatIngredientCategoryReply(
  category: string,
  items: IngredientItem[],
): string {
  if (!items.length) return `Belum ada data bahan kategori ${category}.`;
  return [
    `📋 Stok bahan kategori ${category.toLowerCase()}:`,
    ...items.map(
      (item, i) =>
        `${i + 1}. ${item.name} - stok ${item.stock} ${item.unit ?? ""}`,
    ),
  ].join("\n");
}

export function formatSingleIngredientStockReply(
...
export function formatLowStockIngredientsReply(
  items: IngredientItem[],
): string {
  if (!items.length)
  ...
  return [
    ...
    ...items.map(
      (item, i) =>
        `${i + 1}. ${item.name} (${item.category ?? "-"}) - ${item.stock} ${item.unit ?? ""}`,
    ),
  ].join("\n");
}

export function formatOutOfStockSummaryReply(
  globalStats: any,
  items: IngredientItem[],
): string {
  ...
  if (items.length) {
    ...
    items.forEach((item, i) =>
      lines.push(`${i + 1}. ${item.name} - ${item.stock}`),
    );
  }
  return lines.join("\n");
}

export function formatPopIceAvailabilityReply(items: any[]): string {
  if (!items.length)
    return "Data varian Pop Ice belum tersedia di database bahan.";
  return [
    "🥤 Ketersediaan varian Pop Ice:",
    ...items.map(
      (item, i) => `${i + 1}. ${item.flavor ?? item.name} - ${item.stock}`,
    ),
  ].join("\n");
}
