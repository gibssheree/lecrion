...
import { usePageTitleValue } from "../../app/PageTitleContext";
...
function canShowItem(
  ...
  const moduleOk =
    item.requiredModule === "platform.support"
...
export default function PosLayout({ children }: Props) {
  ...
  // System notifications (promo, fitur, maintenance)
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (
        bizInfoRef.current &&
        !bizInfoRef.current.contains(e.target as Node)
      ) {
  ...
  // Ensure only one navbar dropdown is open at a time
  function openOnly(which: "notif" | "more" | "profile") {
  ...
  // Page title comes from the active page via PageTitleContext
    ...
    >
      ...
      <header className="pos-navbar">
        ...
        <div className="pos-navbar-actions">
          ...
          {/* Bell — notifikasi sistem */}
          <div className="pos-navbar-popover-anchor">
            <button
              ref={notifTriggerRef}
              type="button"
              className={`pos-navbar-icon-btn${notifOpen ? " is-active" : ""}`}
              aria-label="Notifikasi"
              aria-expanded={notifOpen}
              onClick={() =>
                notifOpen ? setNotifOpen(false) : openOnly("notif")
              }
            >
              <Bell size={18} />
              {unreadCount > 0 && (
                <span
                  className="pos-navbar-badge"
          ...
          <div className="pos-navbar-popover-anchor">
            <button
              ...
              type="button"
              className={`pos-navbar-user pos-navbar-user--button${profileOpen ? " is-active" : ""}`}
              aria-label="Profil & pengaturan akun"
              aria-expanded={profileOpen}
              onClick={() =>
                profileOpen ? setProfileOpen(false) : openOnly("profile")
              }
            >
              ...
              <ChevronDown
                size={13}
                className={`pos-navbar-user-chev${profileOpen ? " is-open" : ""}`}
              />
            </button>
            <PopoverMenu
              open={profileOpen}
          ...
          {/* Titik 3 — pengaturan lainnya */}
          <div className="pos-navbar-popover-anchor">
            <button
              ref={moreTriggerRef}
              type="button"
              className={`pos-navbar-icon-btn${moreOpen ? " is-active" : ""}`}
              aria-label="Pengaturan lainnya"
              aria-expanded={moreOpen}
              onClick={() => (moreOpen ? setMoreOpen(false) : openOnly("more"))}
            >
              ...
              <div className="pos-biz-panel">
                ...
                <div className="pos-biz-panel-row">
                  ...
                  <span
                    ...
                    style={{
                      color:
                        STATUS_CONFIG[verificationStatus]?.color ?? "#94a3b8",
                    }}
                  >
                    <span
                    ...
                    />
                    {STATUS_CONFIG[verificationStatus]?.label ??
                      verificationStatus}
                  </span>
                </div>
        ...
        <main className="pos-content">
          ...
          {pageTitle && (
            <div className="pos-page-header">
              <h2 className="pos-page-heading">{pageTitle}</h2>
            </div>
          )}
