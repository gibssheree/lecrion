    ...
    preview: (
      <div className="display-preview display-preview--standard">
        <div className="dp-items">
          <div className="dp-item"><span>Kopi Susu</span><span>×2</span><span>Rp 28.000</span></div>
          <div className="dp-item"><span>Roti Bakar</span><span>×1</span><span>Rp 18.000</span></div>
        </div>
        <div className="dp-total"><span>Total</span><strong>Rp 46.000</strong></div>
      </div>
    ),
  },
  {
...
const TOKO_KEYS = ["storeName", "storeAddress", "storePhone", "businessType", "defaultOrderType"];
const KASIR_KEYS = [
  "hideProductStock", "kasirDisplayEnabled", "kasirDisplayType",
  "requireOpeningCash", "requireClosingCash",
  "autoCloseKasir", "autoCloseTime",
  "allowSaveOrder", "kasirPaymentMethods",
];
const NOTIFIKASI_KEYS = [
  "lowStockAlertsEnabled", "lowStockThreshold",
  "lowStockAlertTargets", "adminPhones",
];
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (!validSections.includes(section)) navigate("/settings/toko", { replace: true });
    if (section === "pengguna" && !isOwner) navigate("/settings/toko", { replace: true });
  ...
  const [createForm, setCreateForm]   = useState({ email: "", password: "", role: "cashier" as UserRole });
  ...
  function togglePayment(key: string) {
    ...
    if (idx === -1) methods.push(key); else methods.splice(idx, 1);
    setForm((p) => ({ ...p, kasirPaymentMethods: methods.join(",") }));
  }
  const on    = (key: string) => form[key] === "true";
  const hasPm = (key: string) => (form.kasirPaymentMethods ?? "").split(",").includes(key);
  ...
  function resetSection(keys: string[]) {
    ...
    const d = settingsApi.data as Record<string, string>;
    setForm((p) => ({ ...p, ...Object.fromEntries(keys.map((k) => [k, d[k] ?? ""])) }));
  }
  ...
  function SaveBar({ keys }: { keys: string[] }) {
    return (
      <div className="settings-save-bar">
        <button type="button" className="btn btn-ghost" onClick={() => resetSection(keys)}>
    ...
    <PosAppShell title="Pengaturan">
      <div className="settings-layout">
        ...
        <nav className="settings-sidenav">
          {NAV_SECTIONS.filter((s) => s.id !== "pengguna" || isOwner).map(({ id, label, Icon }) => (
            <NavLink
              key={id}
              to={`/settings/${id}`}
              className={({ isActive }) =>
        ...
        <div className="settings-module">
          ...
          {/* ── TOKO ──────────────────────────────────────────── */}
          {section === "toko" && (
            <form className="settings-card" onSubmit={(e) => handleSave(e, TOKO_KEYS)}>
              ...
              <div className="settings-card-body">
                <div className="settings-form-grid">
                  <div className="form-group">
                    ...
                    <input className="form-input" value={form.storeName ?? ""} onChange={(e) => setField("storeName", e.target.value)} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Alamat</label>
                    <input className="form-input" value={form.storeAddress ?? ""} onChange={(e) => setField("storeAddress", e.target.value)} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">No. Telepon</label>
                    <input className="form-input" value={form.storePhone ?? ""} onChange={(e) => setField("storePhone", e.target.value)} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Kategori Bisnis</label>
                    <select className="form-input" value={form.businessType ?? "general"} onChange={(e) => setField("businessType", e.target.value)}>
                  ...
                  <div className="form-group">
                    ...
                    <select className="form-input" value={form.defaultOrderType ?? "pickup"} onChange={(e) => setField("defaultOrderType", e.target.value)}>
          ...
          {/* ── KASIR ─────────────────────────────────────────── */}
          {section === "kasir" && (
            <form className="settings-card" onSubmit={(e) => handleSave(e, KASIR_KEYS)}>
              <div className="settings-card-header">
                <Settings2 size={14} /> Pengaturan Kasir
              </div>

              <div className="ks-section-label"><Monitor size={12} /> Tampilan</div>
              <div className="ks-row">
                <div className="ks-row-info">
                  <span className="ks-row-title">Sembunyikan Stok Produk</span>
                  <span className="ks-row-desc">Jumlah stok tidak tampil di kartu produk kasir</span>
                </div>
                <button type="button" className={`ks-toggle${on("hideProductStock") ? " on" : ""}`} onClick={() => toggle("hideProductStock")} />
              </div>
              <div className="ks-row">
                <div className="ks-row-info">
                  <span className="ks-row-title">Kasir Display</span>
                  <span className="ks-row-desc">Tampilkan layar pelanggan saat transaksi berlangsung</span>
                </div>
                <button type="button" className={`ks-toggle${on("kasirDisplayEnabled") ? " on" : ""}`} onClick={() => toggle("kasirDisplayEnabled")} />
              </div>

              {/* ── Display type selector ── */}
              {on("kasirDisplayEnabled") && (
                <>
                  <p className="ks-section-note" style={{ paddingTop: 0 }}>Pilih tipe tampilan layar pelanggan</p>
                  <div className="display-type-grid">
                    {DISPLAY_TYPES.map(({ key, label, desc, preview }) => (
                      ...
                      >
                        <div className="display-type-preview-wrap">{preview}</div>
                        <div className="display-type-info">
                          <div className="display-type-name">
                            {label}
                            {(form.kasirDisplayType ?? "standard") === key && <Check size={13} />}
                          </div>
              ...
              <div className="ks-section-label"><Clock size={12} /> Pembukaan &amp; Penutupan</div>
              <div className="ks-row">
                <div className="ks-row-info">
                  <span className="ks-row-title">Wajib Input Kas Awal</span>
                  <span className="ks-row-desc">Kasir harus mengisi nominal kas awal saat membuka sesi</span>
                </div>
                <button type="button" className={`ks-toggle${on("requireOpeningCash") ? " on" : ""}`} onClick={() => toggle("requireOpeningCash")} />
              </div>
              <div className="ks-row">
                <div className="ks-row-info">
                  <span className="ks-row-title">Isi Kas Akhir saat Tutup</span>
                  <span className="ks-row-desc">Kasir wajib input nominal kas akhir saat menutup sesi</span>
                </div>
                <button type="button" className={`ks-toggle${on("requireClosingCash") ? " on" : ""}`} onClick={() => toggle("requireClosingCash")} />
              </div>
              <div className="ks-row">
                <div className="ks-row-info">
                  <span className="ks-row-title">Tutup Kasir Otomatis</span>
                  <span className="ks-row-desc">Sesi kasir otomatis ditutup pada jam yang ditentukan</span>
                </div>
                <div className="ks-row-controls">
                  {on("autoCloseKasir") && (
                    <input type="time" className="ks-time-input" value={form.autoCloseTime ?? "00:00"} onChange={(e) => setField("autoCloseTime", e.target.value)} />
                  )}
                  <button type="button" className={`ks-toggle${on("autoCloseKasir") ? " on" : ""}`} onClick={() => toggle("autoCloseKasir")} />
                </div>
              </div>

              <div className="ks-section-label"><ShoppingBag size={12} /> Operasional</div>
              <div className="ks-row">
                <div className="ks-row-info">
                  <span className="ks-row-title">Simpan Order</span>
                  <span className="ks-row-desc">Kasir dapat menyimpan pesanan sebelum melakukan pembayaran</span>
                </div>
                <button type="button" className={`ks-toggle${on("allowSaveOrder") ? " on" : ""}`} onClick={() => toggle("allowSaveOrder")} />
              </div>

              <div className="ks-section-label"><Wallet size={12} /> Metode Pembayaran</div>
              <p className="ks-section-note">Pilih metode pembayaran yang ditampilkan di kasir</p>
              <div className="ks-payment-grid" style={{ padding: "4px 20px 8px" }}>
                {PAYMENT_METHODS.map(({ key, label }) => (
                  <button key={key} type="button" className={`ks-payment-chip${hasPm(key) ? " active" : ""}`} onClick={() => togglePayment(key)}>
                    {hasPm(key) && <Check size={11} />}{label}
                  </button>
                ))}
              </div>
          ...
          {/* ── NOTIFIKASI ────────────────────────────────────── */}
          {section === "notifikasi" && (
            <form className="settings-card" onSubmit={(e) => handleSave(e, NOTIFIKASI_KEYS)}>
              <div className="settings-card-header">
              ...
              <div className="ks-row">
                <div className="ks-row-info">
                  ...
                  <span className="ks-row-desc">Kirim notifikasi WhatsApp saat stok produk di bawah ambang</span>
                </div>
                <button type="button" className={`ks-toggle${on("lowStockAlertsEnabled") ? " on" : ""}`} onClick={() => toggle("lowStockAlertsEnabled")} />
              </div>
              ...
              <div className="settings-card-body" style={{ paddingTop: 8 }}>
                <div className="settings-form-grid">
                  <div className="form-group">
                    ...
                    <input type="number" className="form-input" value={form.lowStockThreshold ?? ""} onChange={(e) => setField("lowStockThreshold", e.target.value)} placeholder="misal: 5" />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Nomor WA Alert <span className="form-label-hint">(pisahkan koma)</span></label>
                    <input type="text" className="form-input" value={form.lowStockAlertTargets ?? ""} onChange={(e) => setField("lowStockAlertTargets", e.target.value)} placeholder="628111..., 628222..." />
                  </div>
                  <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                    <label className="form-label">Nomor WA Admin <span className="form-label-hint">boleh akses laporan &amp; stok via WhatsApp — pisahkan koma</span></label>
                    <input type="text" className="form-input" value={form.adminPhones ?? ""} onChange={(e) => setField("adminPhones", e.target.value)} placeholder="628111..., 628222..." />
                  </div>
                </div>
              </div>
              ...
              <div className="settings-card-body">
                <div className="settings-form-grid settings-form-grid--3">
                  <div className="form-group">
                    ...
                    <input className="form-input" readOnly value={BUSINESS_TYPE_LABELS[profileApi.data?.verifiedBusinessVertical ?? ""] ?? profileApi.data?.verifiedBusinessVertical ?? "general"} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Status Verifikasi</label>
                    <input className="form-input" readOnly value={profileApi.data?.verificationStatus ?? "unverified"} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Request Aktif</label>
                    <input className="form-input" readOnly value={BUSINESS_TYPE_LABELS[profileApi.data?.requestedBusinessVertical ?? ""] ?? profileApi.data?.requestedBusinessVertical ?? "-"} />
                  </div>
                </div>
                <div className="settings-request-row">
                  <div className="form-group" style={{ flex: 1 }}>
                    <label className="form-label">Ajukan Kategori Bisnis</label>
                    <select className="form-input" value={requestedVertical} onChange={(e) => setRequestedVertical(e.target.value)}>
                      {BUSINESS_VERTICAL_OPTIONS.map((o) => (
                        <option key={o.value} value={o.value}>{o.label}</option>
                      ))}
                    </select>
                  </div>
                  <button type="button" className="btn btn-primary settings-request-btn" onClick={handleRequestVertical} disabled={requestingVertical || profileApi.loading}>
                    {requestingVertical ? <div className="spinner" style={{ width: 14, height: 14 }} /> : <Send size={13} />}
                    Ajukan
                  </button>
                </div>
              </div>
          ...
          {section === "pengguna" && !isOwner && (
            <div className="settings-card" style={{ padding: 48, textAlign: "center" }}>
          ...
          {section === "pengguna" && isOwner && (
            <div className="settings-card">
              <div className="settings-card-header">
                ...
                <button type="button" className="btn btn-primary btn-sm" style={{ marginLeft: "auto" }} onClick={() => setShowCreate((v) => !v)}>
              ...
              {userError && (
                <div className="alert alert-error" style={{ margin: "12px 20px 0", display: "flex", alignItems: "center", gap: 8 }}>
                  {userError}
                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => setUserError(null)} style={{ marginLeft: "auto" }}><X size={12} /></button>
                </div>
              )}
              ...
              {showCreate && (
                <form onSubmit={submitCreate} className="management-form">
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="password" minLength={6} value={createForm.password} onChange={(e) => setCreateForm((p) => ({ ...p, password: e.target.value }))} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Role</label>
                    <select className="form-select" value={createForm.role} onChange={(e) => setCreateForm((p) => ({ ...p, role: e.target.value as UserRole }))}>
                      {VALID_ROLES.map((r) => <option key={r} value={r}>{ROLE_LABELS[r]}</option>)}
                    </select>
                  </div>
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
                    <button type="button" className="btn btn-ghost btn-sm" onClick={() => setShowCreate(false)}><X size={13} /> Batal</button>
                    <button type="submit" className="btn btn-primary btn-sm" disabled={userSaving}><Save size={13} /> Simpan</button>
                  </div>
                </form>
              )}
              ...
              <div className="dashboard-card-body" style={{ padding: 0 }}>
                {usersApi.loading ? (
                  ...
                  <table className="pos-data-table">
                    <thead>
                      <tr><th>#</th><th>Email</th><th>Role</th><th>Dibuat</th><th>Aksi</th></tr>
                    </thead>
                    <tbody>
                          ...
                          <td>
                            {editingId === user.id
                              ? <div style={{ display: "flex", gap: 6 }}>
                                  <button type="button" className="btn btn-primary btn-sm" disabled={userSaving} onClick={() => saveRole(user)}>Simpan</button>
                                  <button type="button" className="btn btn-ghost btn-sm" onClick={() => setEditingId(null)}>Batal</button>
                                </div>
                              : <button type="button" className="btn btn-ghost btn-sm" onClick={() => { setEditingId(user.id); setEditRole(user.role as UserRole); }}>Edit Role</button>
                            }
                          </td>
                        </tr>
                      ))}
                      {!usersRows.length && (
                        <tr><td colSpan={5} style={{ textAlign: "center", color: "var(--text-muted)" }}>Belum ada pengguna</td></tr>
                      )}
                    </tbody>
                  </table>
                )}
